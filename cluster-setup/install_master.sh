...
if [ "$(uname -m)" = "aarch64" ]; then PLATFORM=arm64; fi
curl -L --fail --remote-name-all https://github.com/cilium/cilium-cli/releases/download/${CILIUM_CLI_VERSION}/cilium-linux-${PLATFORM}.tar.gz{,.sha256sum}
sha256sum --check cilium-linux-${PLATFORM}.tar.gz.sha256sum
sudo tar xzvfC cilium-linux-${PLATFORM}.tar.gz /usr/local/bin
rm cilium-linux-${PLATFORM}.tar.gz{,.sha256sum}
